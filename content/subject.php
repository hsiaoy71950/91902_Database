...
$subject_find = $_REQUEST['subjectID'];

$find_sql= "SELECT * FROM `quote_table` JOIN author_table ON (`author_table`.`Author_ID` = `quote_table`.`Author_ID`) 
WHERE `quote_table`.`Subject1_ID` = $subject_find
OR `quote_table`.`Subject2_ID` = $subject_find
OR `quote_table`.`Subject3_ID` = $subject_find";
